...
function getPatternForSlug(slug: string): typeof coverPatterns[0] {
	...
	const index = slug.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % coverPatterns.length
	return coverPatterns[index]
}

export function PostCover({ title, slug }: Props) {
	const pattern = getPatternForSlug(slug)
	
	return (
		<div 
			className="relative aspect-[16/9] overflow-hidden rounded-lg flex items-center justify-center p-8 transition-transform duration-300 group-hover:scale-[1.02]"
			style={{
