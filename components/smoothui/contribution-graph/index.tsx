...
const CONTRIBUTION_COLORS = [
  "bg-muted/30 dark:bg-muted/10", // Level 0 - No contributions
  "bg-green-600/30 dark:bg-green-500/30", // Level 1
  "bg-green-600/50 dark:bg-green-500/50", // Level 2
  "bg-green-600/70 dark:bg-green-500/70", // Level 3
  "bg-green-600 dark:bg-green-500", // Level 4 - Max
];
...
export function ContributionGraph({
  ...
  return (
    <div className={`contribution-graph ${className}`}>
        ...
        <table className="border-separate border-spacing-[3px] text-xs">
          ...
          <thead>
            <tr className="h-4">
              ...
              {monthHeaders.map((header) => (
                <td
                  className="relative text-left text-foreground/70"
                  colSpan={header.colspan}
                  key={`${header.month}-${header.startWeek}`}
                >
                  <span className="absolute top-0 left-1 text-xs font-medium">{header.month}</span>
                </td>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {Array.from({ length: DAYS_IN_WEEK }, (_, dayIndex) => (
              <tr className="h-3" key={DAYS[dayIndex]}>
                {/* Day Labels */}
                <td className="relative w-8 min-w-8 text-foreground/70">
                  {dayIndex % 2 === 0 && (
                ...
                {Array.from({ length: WEEKS_IN_YEAR }, (_, w) => {
                  ...
                  if (!dayData?.date) {
                    return (
                      <td className="h-3 w-3 p-0" key={cellKey}>
                  ...
                  return (
                    ...
                    <td
                      className="h-3 w-3 cursor-pointer p-0"
                      key={cellKey}
      ...
      {/* Legend */}
      {showLegend && (
        <div className="mt-4 flex items-center justify-end gap-2 text-foreground/70 text-xs">
          <span>Less</span>
